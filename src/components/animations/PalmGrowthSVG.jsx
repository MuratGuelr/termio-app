...
// PalmGrowthSVG renders an inline SVG palm tree with a bottom-to-top mask-based growth reveal.
// It also draws a ground ellipse and an optional mini progress badge on the ellipse rim.
// Props:
// - growth: 0..1 eased growth for plant (use displayedGrowth to freeze during breaks)
// - miniProgress: 0..1 overall timer progress (can continue during breaks)
// - timerColor: stroke color for mini badge progress
// - className: optional additional class names (defaults to 'plant-svg')
export default function PalmGrowthSVG({ growth = 0, miniProgress = 0, timerColor = '#10b981', className = 'plant-svg', palmScale = 0.7 }) {
  const maskId = useMemo(() => `mask-${Math.random().toString(36).slice(2)}`, []);
  // Smoothen growth with internal tween to avoid stepping between discrete updates
  const [animGrowth, setAnimGrowth] = useState(growth);
  const rafRef = useRef(null);
  const startRef = useRef({ from: growth, to: growth, t0: 0 });
  ...
  useEffect(() => {
    ...
    startRef.current = { from: animGrowth, to: growth, t0: now };
  ...
  // Ground and mini badge geometry
  const rx = 42, ry = 9; const cx = 60, cy = 100; const miniR = 9; const theta = Math.PI * 0.15;
  const miniCx = cx + rx * Math.cos(theta);
  const miniCy = cy + ry * Math.sin(theta) - miniR * 0.2;
  const start = -Math.PI / 2;
  const end = start + Math.max(0, Math.min(1, miniProgress)) * Math.PI * 2;

  // Map the external 500x500 SVG into our 120x120 viewBox.
  // Uniform scale to fit height ~ 120, then shift to sit on ground ellipse.
  const s = (120 / 500) * palmScale; // base scaled down
  // After scaling, the bottom (y≈500) maps to ~120. Shift up so trunk base sits near cy=100.
  // We'll translate so the palm group bottom aligns around y=100, and horizontally center near x=60.
  const tx = 60 - (250 * s);
  const ty = 100 - (500 * s);

  // Mask rectangle grows from ground (cy=100) upwards so growth maps 0..1 to full session
  const mh = 100 * Math.max(0, Math.min(1, animGrowth));
  const my = 100 - mh; // start at ground line

  return (
    <svg viewBox="0 0 120 120" className={className} aria-hidden="true" focusable="false">
      <defs>
        <radialGradient id="groundGradPalm" cx="50%" cy="78%" r="60%">
          <stop offset="0%" stopColor="rgba(16,185,129,0.35)" />
          <stop offset="100%" stopColor="rgba(16,185,129,0.10)" />
        </radialGradient>
        <mask id={maskId}>
          <rect x="0" y={my} width="120" height={mh} fill="#fff" />
        </mask>
      </defs>

      {/* Ground ellipse */}
      <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="url(#groundGradPalm)" />

      {/* Mini progress badge on rim */}
      <g opacity="0.95">
        <circle cx={miniCx} cy={miniCy} r={miniR} fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2" />
        <path d={`M ${miniCx} ${miniCy} m 0 -${miniR} A ${miniR} ${miniR} 0 ${miniProgress>0.5?1:0} 1 ${miniCx + miniR*Math.cos(end)} ${miniCy + -miniR*Math.sin(end)}`}
              stroke={timerColor} strokeWidth="3" strokeLinecap="round" fill="none" />
        <circle cx={miniCx} cy={miniCy} r="1.6" fill="rgba(255,255,255,0.85)" />
      </g>

      {/* Palm group, masked to grow from bottom */}
      <g mask={`url(#${maskId})`}>
        <g transform={`translate(${tx} ${ty}) scale(${s})`}>
          {/* Trunk (st4 fill #C98C40) */}
          <path fill="#C98C40" d="M236.38,125.98c0,0-41.82,165.77,0,362.2h45.82c0,0-64.41-156.52-20.81-362.2H236.38z" />
          {/* Canopy (st5 fill #15AA78) */}
